...
# =========================================================
# ENV LOADING + VISIBILITY CHECK
# =========================================================
print(">>> [entity_extract] Loading .env file")
load_dotenv()

print(">>> [entity_extract] GROQ_API_KEY =", os.getenv("GROQ_API_KEY"))
print(">>> [entity_extract] OPENAI_API_KEY =", os.getenv("OPENAI_API_KEY"))
...
def extract_entities_with_ai(text):
    print("\n===================================================")
    print(">>> extract_entities_with_ai() CALLED")
    print(">>> OCR text length:", len(text))
    print("===================================================")

    if not text or len(text.strip()) == 0:
        print("⚠️ OCR text empty → skipping AI → regex fallback")
        return regex_fallback(text)

    try:
        print(">>> Creating OpenAI (Groq) client")
        client = OpenAI(
            base_url="https://api.groq.com/openai/v1"
        )
        ...
        prompt = f"""
You are an expert at extracting data from Indian ID cards.
...
"""
        ...
        json_match = re.search(r'\{.*\}', result, re.DOTALL)
        if not json_match:
            print("⚠️ No JSON detected in AI output → fallback")
            return regex_fallback(text)

        parsed = json.loads(json_match.group(0))
        print(">>> Parsed JSON from AI:", parsed)

        normalized = normalize_and_validate(parsed, text)
        print(">>> Normalized + validated output:", normalized)
...
# =========================================================
def normalize_and_validate(parsed, full_text):
    print(">>> normalize_and_validate() called")
    ...
    if out["aadhar"]:
        ...
        if re.fullmatch(r"\d{12}", a):
            ...
            print("⚠️ Invalid Aadhaar detected")
            out["aadhar"] = None

    # PAN
    if out["pan"]:
        if not re.fullmatch(r"[A-Z]{5}\d{4}[A-Z]", str(out["pan"]).strip()):
            print("⚠️ Invalid PAN detected")
            out["pan"] = None
...
def normalize_dob(dob_str):
    ...
    try:
        ...
        print("⚠️ DOB parsing failed")

    return None

# =========================================================
# REGEX FALLBACK (SAFE MODE)
# =========================================================
def regex_fallback(text):
    print(">>> regex_fallback() called")
    ...
    # Aadhaar
    m = re.search(r"\b(\d{4}\s?\d{4}\s?\d{4})\b", clean)
    if m:
        data["aadhar"] = m.group(1).replace(" ", "")

    # PAN (with context)
    m = re.search(r"\b([A-Z]{5}\d{4}[A-Z])\b", clean)
    if m:
        ctx = clean[max(0, m.start()-50):m.end()+50]
        if re.search(r"PAN|Income Tax|Permanent Account", ctx, re.I):
            data["pan"] = m.group(1)

    # DOB
    m = re.search(r"\b(\d{2}[\/\-]\d{2}[\/\-]\d{4})\b", clean)
    if m:
        data["dob"] = normalize_dob(m.group(1))
    ...
    # Name (uppercase sequences)
    m = re.search(r"\b([A-Z]{3,}(?:\s+[A-Z]{3,})+)\b", clean)
    if m:
        data["name"] = m.group(1).title()

    # Address
    m = re.search(r"(VTC.*?PIN Code\s*\d{6})", clean, re.I)
    if m:
        data["address"] = m.group(1)

    print(">>> regex_fallback output:", data)
    return data
